'''
Zoinks: a program to retrieve field values from Zotero records
...
class MainBody():
    ...
    def _do_preflight(self):
        ...
        if any(item.startswith('-') for item in self.records):
            bad = next(item for item in self.records if item.startswith('-'))
